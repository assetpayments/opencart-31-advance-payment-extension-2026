...
class ControllerExtensionPaymentAssetPayments extends Controller {
	public function index() {
		...
		$advance_payment_setting = trim($this->config->get('payment_assetpayments_advance'));
		...
		$advance_product_title = trim($this->config->get('payment_assetpayments_advance_product_title'));
		$products_data = array();
		...
		$send_data = array(
			...
			'ProcessingId'            => $this->config->get('payment_assetpayments_processingId'),
			'SkipCheckout'            => ($this->config->get('payment_assetpayments_skip_checkout') == 1) ? true : false
		);
	...
	public function callback() {
        ...
        if ($status == 1) {
            // Success
            $target_status_id = $this->config->get('payment_assetpayments_order_status_id');
            $this->model_checkout_order->addOrderHistory($order_id, $target_status_id, 'AssetPayments TransactionID: ' . $transactionId);
        } elseif ($status == 2) {
            // Explicitly Declined
            $declined_status_id = $this->config->get('payment_assetpayments_declined_status_id');
            $this->model_checkout_order->addOrderHistory($order_id, $declined_status_id, 'Payment DECLINED TransactionID: ' . $transactionId);
        } else {
            // Default / Catch-All for any other status
            $default_status_id = $this->config->get('payment_assetpayments_default_status_id');
            $this->model_checkout_order->addOrderHistory($order_id, $default_status_id, 'Payment Status Code: ' . $status . ' | TransactionID: ' . $transactionId);
        }
    }
	}
}
