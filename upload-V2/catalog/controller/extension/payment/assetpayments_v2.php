...
class ControllerExtensionPaymentAssetPaymentsV2 extends Controller {
	...
	public function callback() {
        ...
        if ($status == 1) {
            ...
            $this->model_checkout_order->addOrderHistory($order_id, $target_status_id, 'AssetPayments V2 TransactionID: ' . $transactionId);
        } elseif ($status == 2) {
            // Explicitly Declined
            $declined_status_id = $this->config->get('payment_assetpayments_v2_declined_status_id');
            $this->model_checkout_order->addOrderHistory($order_id, $declined_status_id, 'Payment DECLINED TransactionID: ' . $transactionId);
        } else {
            // Default / Catch-All
            $default_status_id = $this->config->get('payment_assetpayments_v2_default_status_id');
